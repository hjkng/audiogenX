...
class StreamingMultiheadAttention(StreamingModule):
    ...
    def _complete_kv(self, k, v):
        ...
        if self._streaming_state:
            pk = self._streaming_state['past_keys'].to(k.device)
            nk = torch.cat([pk, k], dim=time_dim)

            if v is k:
                nv = nk
            else:
                pv = self._streaming_state['past_values'].to(v.device)
        ...
        if self._is_streaming:
            ...
            # self._streaming_state['past_keys'].detach()
            if v is not k:
                self._streaming_state['past_values'] = nv[:, offset:]
                # self._streaming_state['past_values'].detach()
    ...
    def save_attn_cam(self, cam):
        self.attn_cam = torch.cat([self.attn_cam, cam.cpu()], dim=0) if self.attn_cam is not None else cam.cpu()
    ...
    def save_attn(self, attn):
        ...
        if self.attn is not None:
            self.attn.to(attn.device)
        if self.cross_attention:
            self.attn = torch.cat([self.attn, attn[:b//2, :, :, :]], dim=0) if self.attn is not None else attn[:b//2, :, :, :]
        else:
            self.attn = torch.cat([self.attn, attn[:b//2, :, :, -1:]], dim=0) if self.attn is not None else attn[:b//2, :, :, -1:]
        self.attn.cpu()
    ...
    def save_attn_gradients(self, attn_gradients):
        b, h, _, s = attn_gradients.shape
        if self.attn_gradients is not None:
            self.attn_gradients.to(attn_gradients.device)
        if self.cross_attention:
            self.attn_gradients = torch.cat([self.attn_gradients, attn_gradients[:b//2, :, :, :]], dim=0) if self.attn_gradients is not None else attn_gradients[:b//2, :, :, :]
        else:
            self.attn_gradients = torch.cat([self.attn_gradients, attn_gradients[:b//2, :, :, -1:]], dim=0) if self.attn_gradients is not None else attn_gradients[:b//2, :, :, -1:]
        self.attn_gradients.cpu()

    def get_attn_gradients(block):
        return block.attn_gradients

    # Add text mask
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
                key_padding_mask=None, need_weights=False, attn_mask=None,
                average_attn_weights=True, is_causal=False, is_training=False, text_mask=None):
        ...
        if self.custom:
            ...
            if self.memory_efficient:
                ...
                if _efficient_attention_backend == 'torch':
                    # Use custom attention function to adapt text mask
                    x, attn_weights = scaled_dot_product_attention(
                        q, k, v, is_causal=attn_mask is not None, dropout_p=p, is_training=is_training, text_mask=text_mask)
                ...
                # If we do not use memory_efficient, adapt mask at here
...
class StreamingTransformerLayer(nn.TransformerEncoderLayer):
    ...
    def _cross_attention_block(self, src: torch.Tensor,
                               ...
                               is_training = False,
                               text_mask: tp.Optional[torch.Tensor] = None) -> torch.Tensor:
                                # text_mask 
        assert self.cross_attention is not None
        # queries are from src, keys and values from cross_attention_src.
        x, _ = self.cross_attention(
            src, cross_attention_src, cross_attention_src, need_weights=False, is_training=is_training, text_mask=text_mask)
        return self.dropout_cross(x)  # type: ignore

    # self-attention block
    def _sa_block(self, x: torch.Tensor,
                  attn_mask: torch.Optional[torch.Tensor], key_padding_mask: torch.Optional[torch.Tensor], is_causal: bool = False) -> torch.Tensor:
            ...
            if cross_attention_src is not None:
                x = x + self.layer_scale_cross(
                    self._cross_attention_block(
                        self.norm_cross(x), cross_attention_src, is_training=is_training, text_mask=text_mask))
                        # text_mask 
            ...
            if cross_attention_src is not None:
                x = self.norm_cross(
                    x + self.layer_scale_cross(
                        self._cross_attention_block(src, cross_attention_src, text_mask=text_mask)))
...
class StreamingTransformer(StreamingModule):
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs):
        ...
        if is_training:
            offsets = torch.arange(0, B, device=x.device, dtype=torch.long)
        elif 'offsets' in self._streaming_state:
